...
Status isEmptyList(SequenceList sequenceList);
int getListLength(SequenceList sequenceList);
Status getListElement(SequenceList sequenceList,int index,ListElementType &element);
int  findListElement(SequenceList,ListElementType element);
Status priorElement(SequenceList sequenceList,int index,ListElementType &element);
Status nextElement(SequenceList sequenceList,int index,ListElementType &element);
Status listInsertElement(SequenceList &sequenceList,int index,ListElementType element);
Status listDeleteElement(SequenceList &sequenceList,int index,ListElementType &element);
...
Status initList(SequenceList &sequenceList){
    ...
    sequenceList.element = (ListElementType *) malloc(MAX_SIZE * sizeof(ListElementType)); 
...
*/
int  findListElement(SequenceList sequenceList,ListElementType element){
    if(element == NULL)
        return ERROR;
...
Status priorElement(SequenceList sequenceList,int index,ListElementType &element){
    ...
    return getListElement(sequenceList,index - 1,element);
}
 ...
 * 返回值：插入结果
*/
Status listInsertElement(SequenceList &sequenceList,int index,ListElementType newElement){
    // 如果要插入的位置小于0 
    ...
    if(sequenceList.length >= sequenceList.listSize){
        ...
        //重新申请内存空间
        sequenceList.element = (ListElementType *) realloc(sequenceList.element,newListSize * sizeof(ListElementType));
